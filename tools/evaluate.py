...
class Evaluator():
    def __init__(self, datapath) -> None:
        self.datas = open(datapath, 'r').readlines()
        self.datas = [json.loads(data) for data in self.datas]
        self.references = [data['output'][-1] for data in self.datas]
        self.infers = [data['pred_result'] for data in self.datas]
    ...
    def Rouge(self, candidate, reference):
        ...
        b = ["i am student from school on japan"] #
        '''
        rouge = Rouge()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_path',type=str, default="output/ESConv_ESConv.jsonl")
    parser.add_argument('--cut_flag', default=0, help="#0: no additional process; 1: seg cut; 2: bpe cut", type=int)
    args = parser.parse_args()
    evaluate = Evaluator(args.data_path)
    seg = None
    if args.cut_flag == 1:
        seg = pkuseg.pkuseg()
    ...
    for idx, (ref, inf) in tqdm(zip(evaluate.references, evaluate.infers)):
        reference = []
        resps_num = len(line)
