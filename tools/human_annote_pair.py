...
DIR='/mnt/wsl/PHYSICALDRIVE2/disentangle/dialogue/LLM/Chinese-Vicuna/outs/test/20230511/'
file_path = '/mnt/wsl/PHYSICALDRIVE2/disentangle/dialogue/LLM/Chinese-Vicuna/test/20230508/test_20230507-format2.json'
pre_data = utils.from_json(file_path)
# Files = ['ChatGPT','Wenxin','Ours']
Files = ['Ours','ChatGPT','moss-moon-003','WenXin','Chatglm','Vicuna','Chinese-LLaMA-Alpaca','7b-sharegpt-a100-2'] # 
NUM=len(Files)
...
class ColItem:
    ...
    def load(self, file_or_key):
        if file_or_key=='':
            return 
        if file_or_key=='Ours':
            file_or_key= '7b-sharegpt-4090-2'
...
class Viewer:
    ...
    def save(self, *args):
        ans = []
        col_num = len(self.cols)-1
        scores = args[:col_num]
        comments = args[col_num:]
        for i in range(col_num):
            ans.append({
                'index': self.cols[i+1].col_index,
                'name': self.cols[i+1].name,
                'score': int(scores[i]),
                # 'comment': comments[i],
                'conv': self.cols[i+1].conv(),
            })
        utils.to_jsonl([{
            'comment': comments[0],
            'result': ans,
        }], f'{DIR}/score_pair_{TIME}.jsonl')
    ...
    with gr.Row().style(equal_height=False):        
        for i in range(NUM):
            with gr.Column():
                ...
                info = gr.Markdown()
                chatbot = gr.Chatbot().style(height=2048)
                with gr.Row():
...
demo.launch(share=1,server_port=8084) # server_port=8081
